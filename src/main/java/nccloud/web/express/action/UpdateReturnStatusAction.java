...
public class UpdateReturnStatusAction implements ICommonAction {
    ...
    public Object doAction(IRequest request) {
        try {
            ...
            Integer returnStatus = map != null && map.get("returnStatus") != null
                    ? (Integer) map.get("returnStatus") : null;

            if (pkParcel == null || pkParcel.isEmpty()) {
                throw new Exception("包裹主键不能为空");
            }
            if (returnStatus == null) {
                throw new Exception("退回状态不能为空");
            }

            IExpressReminderService service = ServiceLocator.find(IExpressReminderService.class);
            ParcelVO result = service.updateReturnStatus(pkParcel, returnStatus);
